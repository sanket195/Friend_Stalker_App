...
public class MapFragment extends FragmentActivity implements OnMapReadyCallback {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.signout) {
            Intent i = new Intent(MapFragment.this, LoginActivity.class);
            startActivity(i);
            finish();
        }
...
//        mapFragment.getMapAsync(this);
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        if (intent != null) {
            ...
            maps.addMarker(new MarkerOptions().position(frnlctn).title(email.split("@")[0].substring(0,1).toUpperCase() + email.split("@")[0].substring(1)));
            ...
            for (int i = 0; i < arrayList.size(); i++) {
                ...
                maps.addMarker(new MarkerOptions().position(frnlctn).title(arrayList.get(i).getEmail().split("@")[0].substring(0,1).toUpperCase() + arrayList.get(i).getEmail().split("@")[0].substring(1)));
                maps.moveCamera(CameraUpdateFactory.newLatLng(frnlctn));
            }
